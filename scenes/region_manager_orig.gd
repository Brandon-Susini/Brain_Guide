...
func setupRegions():
	regions = find_child("BrainRegions").get_children().filter(func(r): return r is Polygon2D)
	regions.append($BrainRegions/F4Area/F4)
	regions.append($BrainRegions/F3Area/F3)
...
func _on_guide_screen_type_selected(typeSelected):
	...
	resetRegionColors()
	changeRegionColor(typeSelected)
	pass # Replace with function body.



func _on_f_4_input_event(viewport, event, shape_idx):
	var shape_owner_id: int = find_child("F4Area").shape_find_owner(shape_idx)
	var shape_owner: Object = find_child("F4Area").shape_owner_get_owner(shape_owner_id)
	
	print(shape_owner.name.left(2))
	pass # Replace with function body.


func _on_regions_area_input_event(viewport, event, shape_idx):
	var shape_owner_id: int = find_child("RegionsArea").shape_find_owner(shape_idx)
	var shape_owner: Object = find_child("RegionsArea").shape_owner_get_owner(shape_owner_id)
